...
export default function RegisterScreen({navigation}) {
  ...
  //
  const [typeLockHome, setTypeLockHome] = useState(false);
  const [typeLockVehicle, setTypeLockVehicle] = useState(false);
  const [typeLockSafe, setTypeLockSafe] = useState(false);
  const [array, setArray] = useState([]);
  const [arrayTypeLock, setArrayTypeLock] = useState([]);
  ...
  const onCheckFormInput = async () => {
    ...
    if (name.length > 8 && selectedDate != null && check && array.length > 0) {
      console.log('oke all');
  ...
  function checkTypeLock(string, setType, type) {
  ...
  async function createUser(id) {
    const response = await axios.post(`${baseUrl}/locksmith/create`, {
      _id: id,
      name: name,
      gender: checked,
      ...
      location: {
        type: 'Point',
        coordinates: [106.68733683690445, 10.820834992867436],
      },
      status: true,
    });
          ...
          <View style={styles.bottom}>
            {stateScreen === 'first' && (
              <View style={styles.form}>
                ...
                />
                <Text>Loại khóa bạn có thể sửa</Text>
                <Checkbox.Item
                  label="khóa nhà"
                  color="red"
                  status={typeLockHome ? 'checked' : 'unchecked'}
                  // status={typeLock === 'home' ? 'checked' : 'unchecked'}
                  onPress={() =>
                    checkTypeLock('home', setTypeLockHome, typeLockHome)
                  }
                />
                <Checkbox.Item
                  label="khóa xe"
                  color="red"
                  status={typeLockVehicle ? 'checked' : 'unchecked'}
                  onPress={() =>
                    checkTypeLock(
                      'vehicle',
                      setTypeLockVehicle,
                ...
                <Checkbox.Item
                  label="khóa két sắt"
                  color="red"
                  status={typeLockSafe ? 'checked' : 'unchecked'}
                  onPress={() =>
                    checkTypeLock('safe', setTypeLockSafe, typeLockSafe)
                  }
                />
                <ShowErrorMessage
                  message={'Bạn phải chọn ít nhất 1 loại khóa'}
                  showMessage={showMessage}
